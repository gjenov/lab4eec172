...
void writeCommand(unsigned char c) {
    ...
    GPIOPinWrite(OLED_CS_BASE, OLED_CS_PIN, 0);
...
*/
  ...
  GPIOPinWrite(OLED_RST_BASE, OLED_RST_PIN, 0);	// RESET = RESET_LOW
  ...
  writeCommand(SSD1351_CMD_COMMANDLOCK);  //0xFD  // set command lock
  writeData(0x12);
  writeCommand(SSD1351_CMD_COMMANDLOCK); //0xFD   // set command lock
  writeData(0xB1);
